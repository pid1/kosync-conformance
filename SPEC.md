...
**[K-PUT-7]** The server **MUST NOT** answer **202**. `api.json` declares it as
an accepted status, and no client treats it as success, so a 202 is a push the
client queues for retry while the server believes it stored the position.
...
**[K-ID-16]** An entry in the `PUT` body **MAY** carry `"weak": true`. It means
the client does not consider that identifier sufficient to claim a record that
already exists. The flag appears **only in the request body**: adoption is a
property of a write, so the `ids` parameter of a read has no equivalent and its
grammar is unchanged.

**[K-ID-17]** A `PUT` whose walk resolves **only** through a weak entry **does
not adopt that record.** It writes under `document`, creating it if absent,
exactly as though nothing had matched, and answers with that digest. A record
found through a strong entry is adopted as before. Reads are unaffected: a weak
identifier resolves on a `GET` like any other, and `progress_match` says what the
position is worth.

This is what separates seeding from binding. A copy that shares only a weak
identifier is shown the other copy's position and can seek to its percentage,
and it still keeps a record of its own; it cannot overwrite the position of the
record it read from, and it cannot merge two works that happen to be tagged
alike. Without it, `progress_match` tells a reader not to trust a weak match
while nothing tells a writer not to clobber on one.
...
the only one that can match two genuinely different works, which is why it is
never a binding identifier: it seeds a reader, it does not claim a record. See
`[K-ID-16]` and `[K-ID-17]`.
...
| `[K-FLD-1]` `document` is an opaque non-empty string without a colon | Asserted in consequence rather than directly: every authenticated assertion addresses a document, so an empty one fails them all, and the character set is probed as `[K-DOC-ID-1]` and `[K-DOC-ID-2]`. The colon is refused by the reference's own read route (§14.3) before the field rule can be observed. |
